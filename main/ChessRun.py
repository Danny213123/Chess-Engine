...
def load_images():
    ...
    for piece in pieces:
        chess_images[piece] = py.transform.scale(py.image.load(image_location + piece + ".png"), (square_size, square_size))
...
def main(Engine):
    ...
    # initiate zobrist keys
    # Zobrist_keys.init_zobrist()

    # load zobrist keys
    ZobristKeys.load_zobrist()

    if Engine != "stockfish":
        ChessAlgorithm.import_transposition_table()
    ...
    while running:
        ...
        for e in py.event.get():
            ...
            if e.type == py.QUIT:
                ...
                ZobristKeys.save_zobrist()
                ChessAlgorithm.export_transposition_table()
        ...
        if not gameOver and not human:
            ...
            temp_gs = gs
            ai_move = ChessAlgorithm.find_best_move(temp_gs, valid_moves, Engine)

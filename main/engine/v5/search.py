"""
V4d Search - Enhanced Alpha-Beta with Pruning Optimizations
...
from main.engine.v5.board import BoardState, WP, BP
from main.engine.v5.move_gen import (
    generate_legal_moves, generate_moves, make_move, unmake_move,
    move_to_string, encode_move, decode_move
)
from main.engine.v5.eval import evaluate, INFINITY, MATE_SCORE, DRAW_SCORE
from main.engine.v5.tt import TT, TT_EXACT, TT_ALPHA, TT_BETA
...
# Null Move Pruning (conservative settings)
NULL_MOVE_R = 2  # Reduction depth
NULL_MOVE_MIN_DEPTH = 4  # Minimum depth for NMP (was 3)

# Late Move Reductions (adaptive)
LMR_FULL_DEPTH_MOVES = 4  # Search this many moves at full depth
LMR_REDUCTION_LIMIT = 3   # Minimum depth for LMR

# Multi-Cut pruning
MULTI_CUT_DEPTH = 6        # Minimum depth for multi-cut
MULTI_CUT_MOVES = 6        # Number of moves to test
MULTI_CUT_MARGIN = 3       # Depth reduction for multi-cut search

# Probcut
PROBCUT_DEPTH = 5          # Minimum depth for probcut
PROBCUT_MARGIN = 200       # Score margin for probcut

# History Pruning
HISTORY_PRUNING_DEPTH = 3  # Minimum depth for history pruning
HISTORY_PRUNING_MARGIN = -3000  # History score threshold

# Futility Pruning (conservative)
FUTILITY_MARGIN = [0, 80, 160, 240]  # Per remaining depth (reduced margins)

# Reverse Futility Pruning (Static Null Move)
RFP_MARGIN = [0, 120, 240, 360]  # Per remaining depth

# Razoring
RAZOR_MARGIN = [0, 300, 450, 600]  # Per remaining depth

# Delta Pruning (quiescence)
DELTA_MARGIN = 200  # Big delta for captures

# Aspiration Windows
ASPIRATION_WINDOW = 50  # Initial window size
ASPIRATION_MAX = 400    # Maximum window before full search

# Internal Iterative Deepening
IID_DEPTH = 4  # Minimum depth for IID
...
def get_lmr_reduction(depth: int, move_number: int, pv_node: bool) -> int:
    """
    Calculate LMR reduction based on depth and move number.
    Uses logarithmic formula similar to Stockfish.
    """
    if move_number < LMR_FULL_DEPTH_MOVES or depth < LMR_REDUCTION_LIMIT:
        return 0

    # Logarithmic reduction: log(depth) * log(moveNumber) / 2
    import math
    reduction = int(math.log(depth) * math.log(move_number) / 2.0)
...
def alpha_beta(board: BoardState, depth: int, alpha: int, beta: int,
               info: SearchInfo, ply: int, pv: List[int], do_null: bool = True, prev_move: int = 0) -> int:
    """
    Alpha-Beta with:
    ...
    # Check Extension: Extend search if in check (more selective)
    # Only extend at reasonable depths and not too deep in the tree
    ...
    if entry and entry.depth >= depth and not pv_node:
    ...
    # =========================================================================
    # If our position is so good that even a null move won't drop below beta, prune
    if (depth <= 3 and not in_check and not pv_node and abs(beta) < MATE_SCORE - 100):
    ...
    # RAZORING
    # =========================================================================
    # If position is hopeless, drop into quiescence early
    ...
    # INTERNAL ITERATIVE DEEPENING
    # =========================================================================
    # If no TT move at a PV node, do shallow search to find good move
    if pv_node and depth >= IID_DEPTH and tt_move == 0:
        alpha_beta(board, depth - IID_REDUCTION, alpha, beta, info, ply, pv, do_null, prev_move)

        # Re-probe TT for the move found
        entry = TT.probe(hash_key)
        if entry:
            tt_move = entry.move
    ...
    # PROBCUT
    # =========================================================================
    # If reduced search shows position is far above beta, return early
    if (depth >= PROBCUT_DEPTH and not in_check and not pv_node and
        ...
        probcut_score = -alpha_beta(board, depth - 4, -rbeta, -rbeta + 1,
                                     info, ply + 1, local_pv, do_null=False, prev_move=0)
    ...
    # =========================================================================
    # Skip if: in check, null move disabled, or only pawns/kings left
    if (do_null and depth >= NULL_MOVE_MIN_DEPTH and not in_check and not pv_node):
        # Check if we have major pieces (can't do null move in pawn endings)
        ...
        if has_pieces:
            # Adaptive null move reduction based on depth
            R = NULL_MOVE_R + (depth > 6)  # Extra reduction at high depths
            ...
            # Reduced depth search (no prev_move for null move)
            ...
            if null_score >= beta:
                ...
                if depth > 12:
                    verify = alpha_beta(board, depth - R, beta - 1, beta, info, ply, local_pv, do_null=False, prev_move=prev_move)
                    if verify >= beta:
                        return beta
                else:
    ...
    for i, move in enumerate(moves):
        ...
        if (depth <= 3 and not in_check and not is_cap and not pv_node and
        ...
        if found_pv:
            ...
            score = -alpha_beta(board, depth - 1, -beta, -alpha, info, ply + 1, local_pv, True, move)
...
def search(board: BoardState, time_limit: float = 0, depth_limit: int = MAX_DEPTH,
    ...
    if verbose:
        ...
        print(f"[V4d] Search complete: {info.nodes} nodes in {elapsed:.2f}s "
              f"({nps} nps)")

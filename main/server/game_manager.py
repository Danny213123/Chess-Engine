...
from main.engine.v3.chess_engine import GameState
from main.engine.v3.chess_move import Move
from main.engine.v2 import chess_algorithm as algo_v2
from main.engine.v3 import chess_algorithm as algo_v3
from main.engine.v4 import chess_algorithm as algo_v4
from main.engine.v4b import chess_algorithm as algo_v4b
from main.engine.v4c import chess_algorithm as algo_v4c
from main.engine.v5 import chess_algorithm as algo_v5
from main.engine.v5b import chess_algorithm as algo_v5b
from main.engine.v5c import chess_algorithm as algo_v5c
from main.engine.v5d import chess_algorithm as algo_v5d
from main.engine.v6 import chess_algorithm as algo_v6
...
class GameManager:
    ...
    def set_engine_version(self, version: str, color: str = None):
        ...
        ver = version.lower()
        if ver not in ["v2", "v3", "v4", "v4b", "v4c", "v5", "v5b", "v5c", "v5d", "v6", "stockfish", "human"]:
            return
    ...
    def ai_move(self):
        ...
        if current_engine == "v5":
             ...
             from main.engine.v5.search import SearchInfo
             ...
             from main.engine.v5b.search import SearchInfo
             self.current_search_info = SearchInfo()
             ...
             from main.engine.v5c.search import SearchInfo
             self.current_search_info = SearchInfo()
             ...
             from main.engine.v5d.search import SearchInfo
             self.current_search_info = SearchInfo()
             ...
             if isinstance(result, tuple):
                 ...
                 move = result
        elif current_engine == "stockfish":
             # Stockfish wrapper - lazy import
             from main.engine.stockfish import chess_algorithm as algo_sf
             result = algo_sf.find_best_move(self.gs, self.valid_moves, current_engine)
             if isinstance(result, tuple):
                 move, stats = result
             else:
                 move = result
        ...
        if move:
            ...
            if stats:
                ...
                if is_valid_score and depth > 0:
                    ...
                    # Always normalize to White's perspective (positive = White better, negative = Black better)
                    if current_engine != "stockfish":
                        try:
                            ...
                            # Stockfish returns eval from side-to-move perspective (CP)
                            # We check the FEN to see who is to move
                            parts = fen.split()
                            side_to_move = parts[1] if len(parts) > 1 else 'w'

                            if side_to_move == 'w':
                                # White to move: Score is from White's perspective
                                search_entry["stockfish_eval"] = raw_eval
                            else:
                                # Black to move: Score is from Black's perspective
                                # Example: +100 means Black is winning (White is -100)
                                # So we negate it to get White's perspective (-100)
                                search_entry["stockfish_eval"] = -raw_eval
                                
                        except Exception as e:
                            print(f"[GameManager] Could not get Stockfish eval: {e}")
                            search_entry["stockfish_eval"] = None
                    else:
                        # Stockfish's own evaluation
                        # If engine is Stockfish, it just returned 'score' relative to side-to-move (captured in stats)
                        # We need to normalize current_engine="stockfish" similarly
                        # But wait, 'score' above comes from 'stats' which comes from the engine output just now?
                        # No, the logic above for "stockfish" engine just copies `score` from `stats` to `last_search_stats`?
                        # Actually for current_engine="stockfish", we recorded `score` in `last_search_stats` and `search_entry`.
                        # But `score` there is from side-to-move (because it came from `sf.get_best_move`).
                        # So we need to normalize it here too.
                        ...
                        if side_to_move == 'w':
                            search_entry["stockfish_eval"] = score
                        else:
                            search_entry["stockfish_eval"] = -score
    ...
    def get_best_move(self, start_sq=None):
        ...
        try:
             ...
             # Then filter results.
             top_moves = algo_v3.find_top_moves(self.gs, self.valid_moves, self.engine_version, top_n=3)

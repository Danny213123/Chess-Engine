...
class GameState:
    ...
    def get_king_pos(self, piece_colour):
        if piece_colour == "w":
    ...
    def get_num_attacking_pieces(self, position, enemy):
        ...
        for x in range(8):
            for y in range(8):
                if self.board[x][y][0] == enemy:
                    moves = self.get_valid_moves_at_square(x, y)
                    for move in moves:
    ...
    def get_valid_moves_at_square(self, row, col):
        piece = self.board[row][col][1]
        moves = []
        self.move_function[piece](row, col, moves)
        return moves
    
    def is_file_open(self, col):
        for row in range(8):
            if self.board[row][col] != "--":
                return False
        return True
    
    def is_diagonal_open(self, row, col):
        for x in range(8):
            if row + x < 8 and col + x < 8:
                if self.board[row + x][col + x] != "--":
                    return False
            if row - x >= 0 and col + x < 8:
                if self.board[row - x][col + x] != "--":
                    return False
            if row + x < 8 and col - x >= 0:
                if self.board[row + x][col - x] != "--":
                    return False

"""
V5c Search - Ultra-Fast Alpha-Beta for Maximum Depth
Optimized for speed and depth over accuracy.

Features:
- Ultra-aggressive pruning for maximum depth
...
from chess_engine.engine.v5c.board import BoardState, WP, BP
from chess_engine.engine.v5c.move_gen import (
    generate_legal_moves, generate_moves, make_move, unmake_move,
    move_to_string, encode_move, decode_move
)
from chess_engine.engine.v5c.eval import evaluate, INFINITY, MATE_SCORE, DRAW_SCORE
from chess_engine.engine.v5c.tt import TT, TT_EXACT, TT_ALPHA, TT_BETA
...
# Null Move Pruning (ultra-aggressive for V5c)
NULL_MOVE_R = 4  # Very high reduction for maximum speed
NULL_MOVE_MIN_DEPTH = 2  # Allow very early null move

# Late Move Reductions (ultra-aggressive)
LMR_FULL_DEPTH_MOVES = 2  # Search even fewer moves at full depth
LMR_REDUCTION_LIMIT = 2   # Allow LMR at shallower depths

# Late Move Pruning (ultra-aggressive for V5c)
LMP_DEPTH = 10  # Higher depth threshold
LMP_BASE_MOVES = 2  # Fewer base moves
LMP_MULTIPLIER = 1.5  # Lower multiplier for tighter pruning
...
# Futility Pruning (ultra-aggressive)
FUTILITY_MARGIN = [0, 120, 250, 400, 600]  # Wider margins for more pruning

# Reverse Futility Pruning (ultra-aggressive)
RFP_MARGIN = [0, 80, 180, 300, 450]  # Tighter margins, extended to depth 4
RFP_DEPTH = 4  # Allow RFP at depth 4

# Razoring (ultra-aggressive)
RAZOR_MARGIN = [0, 200, 350, 500]  # Even tighter margins

# Delta Pruning (quiescence)
DELTA_MARGIN = 120  # Even tighter margin

# Aspiration Windows (wider for fewer re-searches)
ASPIRATION_WINDOW = 40  # Wider initial window
ASPIRATION_MAX = 500    # Higher max before full search

# Internal Iterative Deepening (DISABLED for speed)
IID_DEPTH = 999  # Effectively disabled

"""
V5c Chess Algorithm Adapter
Integrates the V5c bitboard engine with the existing GameState interface.
Optimized for maximum depth with ultra-aggressive pruning.
"""

import time
from typing import Optional

from chess_engine.engine.v5c.board import BoardState
from chess_engine.engine.v5c.search import search, get_best_move
from chess_engine.engine.v5c.move_gen import move_to_string, generate_legal_moves
from chess_engine.engine.v3.chess_opening_book import OpeningBook

# Opening book
OPENING_BOOK = OpeningBook()

# Default search parameters - V5c optimized for speed/depth
DEFAULT_TIME_LIMIT = 3.0  # seconds per move
DEFAULT_DEPTH = 20  # target higher depth with faster search


def find_best_move(game_state, valid_moves, engine, search_info=None):
    """
    Entry point for V5c engine.
    ...
    """
    print(f"[V5c] Bitboard Engine - Ultra-Fast Search for Maximum Depth")
    ...
    # 2. Convert to V5c BoardState
    board = BoardState()
    board.from_fen(game_state.get_fen())

    # 3. Run V5c search with optional external search_info
    start = time.time()
    ...
    if best_move_encoded == 0:
        print("[V5c] WARNING: No move found, using first legal move")
    ...
    # 4. Convert V5c move back to V3 Move
    move_str = move_to_string(best_move_encoded)
    score = stats['score']
    print(f"[V5c] Best: {move_str} ({score} cp) in {elapsed:.2f}s")
    ...
    print(f"[V5c] Warning: Could not match move {move_str}, using first legal")
    empty_stats = {'depth': 0, 'nodes': 0, 'time': elapsed, 'score': 0, 'nps': 0, 'pv': ''}

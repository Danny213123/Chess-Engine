...
static constexpr Bitboard CENTER_MASK =
    (1ULL << D4) | (1ULL << E4) | (1ULL << D5) | (1ULL << E5);

static bool has_piece_on(const Board& board, Color color, Piece piece, Square sq) {
    return board.piece_at(sq) == piece && board.color_at(sq) == color;
}

static int developed_minors(const Board& board, Color color) {
    if (color == WHITE) {
        int home = 0;
        if (has_piece_on(board, WHITE, KNIGHT, B1)) home++;
        if (has_piece_on(board, WHITE, KNIGHT, G1)) home++;
        if (has_piece_on(board, WHITE, BISHOP, C1)) home++;
        if (has_piece_on(board, WHITE, BISHOP, F1)) home++;
        return 4 - home;
    }

    int home = 0;
    if (has_piece_on(board, BLACK, KNIGHT, B8)) home++;
    if (has_piece_on(board, BLACK, KNIGHT, G8)) home++;
    if (has_piece_on(board, BLACK, BISHOP, C8)) home++;
    if (has_piece_on(board, BLACK, BISHOP, F8)) home++;
    return 4 - home;
}

static int center_attacks(const Board& board, Color color, Bitboard all_occ) {
    int attacks = 0;

    Bitboard bb = board.pieces_of(color, PAWN);
    while (bb) {
        int sq = pop_lsb(bb);
        attacks += popcount(pawn_attacks[color][sq] & CENTER_MASK);
    }

    bb = board.pieces_of(color, KNIGHT);
    while (bb) {
        int sq = pop_lsb(bb);
        attacks += popcount(knight_attacks[sq] & CENTER_MASK);
    }

    bb = board.pieces_of(color, BISHOP);
    while (bb) {
        int sq = pop_lsb(bb);
        attacks += popcount(bishop_attacks(sq, all_occ) & CENTER_MASK);
    }

    bb = board.pieces_of(color, ROOK);
    while (bb) {
        int sq = pop_lsb(bb);
        attacks += popcount(rook_attacks(sq, all_occ) & CENTER_MASK);
    }

    bb = board.pieces_of(color, QUEEN);
    while (bb) {
        int sq = pop_lsb(bb);
        attacks += popcount((bishop_attacks(sq, all_occ) | rook_attacks(sq, all_occ)) & CENTER_MASK);
    }

    return attacks;
}

static int castling_development_score(const Board& board, Color color) {
    if (color == WHITE) {
        Square king_sq = board.king_square(WHITE);
        if (king_sq == G1 || king_sq == C1) {
            return CASTLED_KING_BONUS;
        }

        bool king_side_ready = (board.castling_rights & Board::WHITE_OO) &&
            board.piece_at(F1) == NO_PIECE && board.piece_at(G1) == NO_PIECE;
        bool queen_side_ready = (board.castling_rights & Board::WHITE_OOO) &&
            board.piece_at(B1) == NO_PIECE && board.piece_at(C1) == NO_PIECE &&
            board.piece_at(D1) == NO_PIECE;
        return (king_side_ready || queen_side_ready) ? CASTLING_READY_BONUS : 0;
    }

    Square king_sq = board.king_square(BLACK);
    if (king_sq == G8 || king_sq == C8) {
        return CASTLED_KING_BONUS;
    }

    bool king_side_ready = (board.castling_rights & Board::BLACK_OO) &&
        board.piece_at(F8) == NO_PIECE && board.piece_at(G8) == NO_PIECE;
    bool queen_side_ready = (board.castling_rights & Board::BLACK_OOO) &&
        board.piece_at(B8) == NO_PIECE && board.piece_at(C8) == NO_PIECE &&
        board.piece_at(D8) == NO_PIECE;
    return (king_side_ready || queen_side_ready) ? CASTLING_READY_BONUS : 0;
}

static int early_major_piece_penalty(const Board& board, Color color, int developed) {
    int undeveloped = std::max(0, 4 - developed);
    if (undeveloped == 0) {
        return 0;
    }

    int penalty = 0;
    Bitboard rooks = board.pieces_of(color, ROOK);
    while (rooks) {
        int sq = pop_lsb(rooks);
        bool home = color == WHITE ? (sq == A1 || sq == H1) : (sq == A8 || sq == H8);
        if (!home) {
            penalty += EARLY_ROOK_MOVE_PENALTY + undeveloped * 4;
        }
    }

    Bitboard queens = board.pieces_of(color, QUEEN);
    while (queens) {
        int sq = pop_lsb(queens);
        bool home = color == WHITE ? sq == D1 : sq == D8;
        if (!home) {
            penalty += EARLY_QUEEN_MOVE_PENALTY + undeveloped * 5;
        }
    }
...
static int rook_activity_weight(bool opening_phase, int developed) {
    return (opening_phase && developed < 3) ? std::max(1, MOBILITY_ROOK / 2) : MOBILITY_ROOK;
}

static int rook_file_bonus(int bonus, bool opening_phase, int developed) {
    return (opening_phase && developed < 3) ? bonus / 2 : bonus;
}
...
int evaluate(const Board& board) {
    ...
    int white_developed_minors = developed_minors(board, WHITE);
    int black_developed_minors = developed_minors(board, BLACK);
    bool opening_phase = phase > (PHASE_TOTAL * 2 / 3);
    int white_rook_mobility_weight = rook_activity_weight(opening_phase, white_developed_minors);
    int black_rook_mobility_weight = rook_activity_weight(opening_phase, black_developed_minors);
    ...
    while (bb) {
        ...
        int mob = popcount(attacks);
        score_mg += mob * white_rook_mobility_weight;
    }
    ...
    while (bb) {
        ...
        int mob = popcount(attacks);
        score_mg -= mob * black_rook_mobility_weight;
    }
    ...
    while (bb) {
        ...
        if (!(white_pawns & f_mask) && !(black_pawns & f_mask)) {
            int bonus = rook_file_bonus(ROOK_OPEN_FILE_BONUS, opening_phase, white_developed_minors);
            score_mg += bonus;
            score_eg += bonus / 2;
        }
        // Semi-open file (no friendly pawns)
        else if (!(white_pawns & f_mask)) {
            int bonus = rook_file_bonus(ROOK_SEMI_OPEN_FILE_BONUS, opening_phase, white_developed_minors);
            score_mg += bonus;
            score_eg += bonus / 2;
        }
    ...
    while (bb) {
        ...
        // Open file
        if (!(white_pawns & f_mask) && !(black_pawns & f_mask)) {
            int bonus = rook_file_bonus(ROOK_OPEN_FILE_BONUS, opening_phase, black_developed_minors);
            score_mg -= bonus;
            score_eg -= bonus / 2;
        }
        // Semi-open file
        else if (!(black_pawns & f_mask)) {
            int bonus = rook_file_bonus(ROOK_SEMI_OPEN_FILE_BONUS, opening_phase, black_developed_minors);
            score_mg -= bonus;
            score_eg -= bonus / 2;
        }
    ...
    if (opening_phase) {
        ...
        white_opening += white_developed_minors * MINOR_DEVELOPMENT_BONUS;
        black_opening += black_developed_minors * MINOR_DEVELOPMENT_BONUS;

        white_opening += popcount(white_pawns & CENTER_MASK) * CENTER_PAWN_BONUS;
        black_opening += popcount(black_pawns & CENTER_MASK) * CENTER_PAWN_BONUS;

        white_opening += center_attacks(board, WHITE, all_occ) * CENTER_CONTROL_BONUS;
        black_opening += center_attacks(board, BLACK, all_occ) * CENTER_CONTROL_BONUS;

        white_opening += castling_development_score(board, WHITE);
        black_opening += castling_development_score(board, BLACK);

        white_opening -= early_major_piece_penalty(board, WHITE, white_developed_minors);
        black_opening -= early_major_piece_penalty(board, BLACK, black_developed_minors);

        score_mg += white_opening - black_opening;
    }
    ...
    // Return score relative to side to move, then add tempo for that side.
    return (board.side_to_move == WHITE ? score : -score) + TEMPO_BONUS;
}

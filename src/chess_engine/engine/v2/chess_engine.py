...
from .chess_castling import castle_rights
...
class GameState:
    def __init__(self):
        ...
        # Legacy board representation (kept for compatibility with visualization/string methods for now)
        self.board = [
        ...
        # Initialize bitboards from the starting board array
    ...
    def _init_bitboards_from_board(self):
        """Initialize bitboards from the 2D board array."""
        # Reset
        for k in self.bitboards: self.bitboards[k] = 0
        for k in self.occupancies: self.occupancies[k] = 0
        
        for rank in range(8):
            for file in range(8):
                piece = self.board[rank][file]
                if piece != "--":
                    # Map 2D array (0,0 is top-left) to Bitboard (0 is bottom-left A1)
                    # Array: rank 0 is 8th rank. rank 7 is 1st rank.
                    # Bitboard: A1=0 (rank 7, col 0 in array context.. wait)
                    # In array: row 7, col 0 is wR (A1). row 7, col 7 is wR (H1).
                    ...
                    # Update occupancies
                    if piece[0] == "w":
                        self.occupancies["w"] = set_bit(self.occupancies["w"], square)
                    else:
                        self.occupancies["b"] = set_bit(self.occupancies["b"], square)
                        
        self.occupancies["both"] = self.occupancies["w"] | self.occupancies["b"]
    ...
    def make_move(self, move) -> None:
        ...
        # Update bitboards
        self._make_move_bitboards(move)
    ...
    def undo_move(self) -> None:
        ...
        if len(self.move_log) < 1:
            ...
            # Update bitboards (Undo)
            self._undo_move_bitboards(move)
    ...
    def get_valid_moves(self) -> list:
        ...
        # Set checkmate and stalemate flags based on available moves
        if len(moves) == 0:
            if self.in_check:
                ...
                if 0 <= end_row < 8 and 0 <= end_col < 8:
                    ...
                    # if end piece is an enemy
                        ...
                        if (0 <= index <= 3 and type == "R") or (4 <= index <= 7 and type == "B") or \
                                (col == 1 and type == "P" and ((enemy == "w" and (index == 5 or index == 7)) or (enemy == "b" and (index == 4 or index == 6)))) or \
    ...
    def _make_move_bitboards(self, move):
        """Update bitboards based on the move."""
        start_sq = (7 - move.start_row) * 8 + move.start_col
        end_sq = (7 - move.end_row) * 8 + move.end_col
        
        piece = move.pieceMoved
        
        # 1. Move piece
        self.bitboards[piece] = pop_bit(self.bitboards[piece], start_sq)
        self.bitboards[piece] = set_bit(self.bitboards[piece], end_sq)
        
        # 2. Handle Captures
        if move.pieceCaptured != "--":
            # Normal capture
            self.bitboards[move.pieceCaptured] = pop_bit(self.bitboards[move.pieceCaptured], end_sq)
        ...
        if move.pawn_promotion:
            ...
            if piece[0] == "w":
                # White captured black pawn. Black pawn was at (start_row, end_col)
                cap_sq = (7 - move.start_row) * 8 + move.end_col
                self.bitboards["bP"] = pop_bit(self.bitboards["bP"], cap_sq)
            else:
                # Black captured white pawn. White pawn was at (start_row, end_col)
                cap_sq = (7 - move.start_row) * 8 + move.end_col
                self.bitboards["wP"] = pop_bit(self.bitboards["wP"], cap_sq)
                
        elif move.isCastleMove:
            # Move rook
            if move.end_col - move.start_col == 2: # Kingside
                # Rook from H to F (files 7 to 5)
                cur_rook_file = 7
                new_rook_file = 5
            else: # Queenside
                # Rook from A to D (files 0 to 3)
                cur_rook_file = 0
                new_rook_file = 3
                
            rank = 7 - move.end_row # 0 for white, 7 for black (Wait: 7-7=0 for white. 7-0=7 for black in array? Check mapping)
            # Array: Row 7 = Rank 1 (White). Row 0 = Rank 8 (Black).
            # Start_row for White King is 7. 
            # (7 - 7) * 8 = 0. Range 0-7 is Rank 1. Correct.
            
            rook_start_sq = rank * 8 + cur_rook_file
            rook_end_sq = rank * 8 + new_rook_file
            
            rook_piece = piece[0] + "R"
            self.bitboards[rook_piece] = pop_bit(self.bitboards[rook_piece], rook_start_sq)
            self.bitboards[rook_piece] = set_bit(self.bitboards[rook_piece], rook_end_sq)

        # 4. Update occupancies
        self._update_occupancies()

    def _undo_move_bitboards(self, move):
        """Undo bitboard updates."""
        start_sq = (7 - move.start_row) * 8 + move.start_col
        end_sq = (7 - move.end_row) * 8 + move.end_col
        ...
        # 1. Move piece back
        self.bitboards[piece] = pop_bit(self.bitboards[piece], end_sq)
        self.bitboards[piece] = set_bit(self.bitboards[piece], start_sq)
        
        # 2. Restore captured piece
        if move.pieceCaptured != "--":
            self.bitboards[move.pieceCaptured] = set_bit(self.bitboards[move.pieceCaptured], end_sq)
            
        # 3. Special Moves
        if move.pawn_promotion:
            # pieceMoved is the pawn. We put it back at start_sq in step 1.
            # But we popped the PROMOTED piece (which was at end_sq) in step 1?
            # NO. step 1 popped `move.pieceMoved` (pawn) from end_sq.
            # But the bitboard at end_sq actually has a QUEEN.
            ...
            # Step 1: set pawn at start_sq. (Correct).
            pass
            ...
            if piece[0] == "w":
                cap_sq = (7 - move.start_row) * 8 + move.end_col
                self.bitboards["bP"] = set_bit(self.bitboards["bP"], cap_sq)
            else:
                cap_sq = (7 - move.start_row) * 8 + move.end_col
                self.bitboards["wP"] = set_bit(self.bitboards["wP"], cap_sq)
            ...
            rook_piece = piece[0] + "R"
            # Remove from new, add to start
            self.bitboards[rook_piece] = pop_bit(self.bitboards[rook_piece], rook_end_sq)
            self.bitboards[rook_piece] = set_bit(self.bitboards[rook_piece], rook_start_sq)

        # 4. Update occupancies
        self._update_occupancies()

    def _update_occupancies(self):
        """Recalculate occupancies from individual bitboards."""
        self.occupancies["w"] = (self.bitboards["wP"] | self.bitboards["wN"] | self.bitboards["wB"] | 
                                 self.bitboards["wR"] | self.bitboards["wQ"] | self.bitboards["wK"])
        self.occupancies["b"] = (self.bitboards["bP"] | self.bitboards["bN"] | self.bitboards["bB"] | 
    ...
    def get_pawn_moves(self, start_row, start_col, moves: list) -> None:
        ...
        if self.white:
            ...
            move_amount = -1
            pawn_start_rank = 6  # Starting rank for white pawns (for double move check)
            enemy = "b"
        ...
        # if black
            ...
            move_amount = 1
            pawn_start_rank = 1  # Starting rank for black pawns (for double move check)
            enemy = "w"
        ...
        if self.white:
            ...
            if self.board[start_row - 1][start_col] == "--":
                ...
                if not piece_pinned or pin_direction == (-1, 0):
                    ...
                    if start_row == pawn_start_rank and self.board[start_row - 2][start_col] == "--":
        ...
        for index in range(8):
            ...
            if 0 <= end_row < 8 and 0 <= end_col < 8:
                ...
                if end_piece[0] != ally:
                    ...
                    # Temporarily move king coords
                    if ally == "w":
                        self.white_king = (end_row, end_col)
                    else:
                        self.black_king = (end_row, end_col)
                    
                    # Temporarily update board to reflect move 
                    # (Critical so checking logic doesn't see old king as blocker)
                    piece_on_start = self.board[start_row][start_col]
                    self.board[start_row][start_col] = "--"
                    self.board[end_row][end_col] = piece_on_start 

                    # check for pins and checks
                    inCheck, pins, checks = self.check_for_pins_and_checks()
                    
                    # Revert board
                    self.board[start_row][start_col] = piece_on_start

"""
V5d Search - Improved Alpha-Beta with 5s Time Limit
Implements improvements from IMPROVEMENT_PLAN.md:
- Enhanced SEE (Static Exchange Evaluation)
- Dynamic quiescence search depth
- Better time management
- Improved move ordering
- Check extensions in quiescence

Features:
- 5 second search time (configurable)
- Full SEE for capture sequence evaluation
- Dynamic QSearch with check extensions
- Improved history and countermove heuristics
...
from chess_engine.engine.v5d.board import BoardState, WP, BP
from chess_engine.engine.v5d.move_gen import (
    generate_legal_moves, generate_moves, make_move, unmake_move,
    move_to_string, encode_move, decode_move
)
from chess_engine.engine.v5d.eval import evaluate, INFINITY, MATE_SCORE, DRAW_SCORE
from chess_engine.engine.v5d.tt import TT, TT_EXACT, TT_ALPHA, TT_BETA
...
# Null Move Pruning (very aggressive for deep V5d search)
NULL_MOVE_R = 5  # Higher reduction for faster search
NULL_MOVE_MIN_DEPTH = 2  # Allow very early null move

# Late Move Reductions (more aggressive for deeper search)
LMR_FULL_DEPTH_MOVES = 2  # Search few moves at full depth
LMR_REDUCTION_LIMIT = 2   # Allow LMR at shallow depths

# Late Move Pruning (more aggressive)
LMP_DEPTH = 12  # Higher depth threshold
LMP_BASE_MOVES = 2  # Fewer base moves
LMP_MULTIPLIER = 1.3  # Tighter pruning for more cutoffs

# Multi-Cut pruning (ultra-aggressive)
MULTI_CUT_DEPTH = 4        # Even lower threshold
MULTI_CUT_MOVES = 4        # Even fewer moves to test
MULTI_CUT_MARGIN = 5       # Even higher reduction

# Probcut (ultra-aggressive)
PROBCUT_DEPTH = 3          # Lower threshold
PROBCUT_MARGIN = 120       # Tighter margin

# Singular Extensions (DISABLED for speed)
SINGULAR_DEPTH = 999       # Effectively disabled
SINGULAR_MARGIN = 64       # Score margin for singularity

# History Pruning (ultra-aggressive)
HISTORY_PRUNING_DEPTH = 5  # Higher depth threshold
HISTORY_PRUNING_MARGIN = -1500  # Even higher threshold

# Futility Pruning (more aggressive for depth)
FUTILITY_MARGIN = [0, 100, 200, 350, 500, 700]  # Extended to depth 5

# Reverse Futility Pruning (aggressive)
RFP_MARGIN = [0, 70, 150, 250, 380, 500]  # Extended to depth 5
RFP_DEPTH = 5  # Allow RFP at depth 5

# Razoring (aggressive)
RAZOR_MARGIN = [0, 180, 320, 450]  # Tighter margins

# Delta Pruning (quiescence)
DELTA_MARGIN = 120  # Even tighter margin

# Aspiration Windows
ASPIRATION_WINDOW = 25  # Narrower initial window for better accuracy
ASPIRATION_MAX = 400    # Max before full search

# Internal Iterative Deepening
IID_DEPTH = 6  # Enable IID at depth 6+
IID_REDUCTION = 2  # Depth reduction for IID search

# Default time limit for V5d (5 seconds)
DEFAULT_TIME_LIMIT = 5.0

# Pre-computed LMR reduction table (more aggressive for V5c)
LMR_TABLE = np.zeros((64, 64), dtype=np.int32)
for d in range(1, 64):
    for m in range(1, 64):
        if d >= 2 and m >= 2:
            # More aggressive formula for deeper reductions
            LMR_TABLE[d][m] = max(1, int(0.5 + math.log(d) * math.log(m) / 1.8))
        else:
            LMR_TABLE[d][m] = 0
...
# =============================================================================
# QUIESCENCE SEARCH WITH CHECK EXTENSIONS (Improved per IMPROVEMENT_PLAN.md)
# =============================================================================

# Maximum quiescence search depth for checks (reduced for speed)
MAX_QSEARCH_CHECK_PLY = 3  # Only search checks up to depth 3 (was 6)
MAX_QSEARCH_PLY = 8        # Reduced hard limit (was 12)

def quiescence(board: BoardState, alpha: int, beta: int, info: SearchInfo, ply: int, q_ply: int = 0) -> int:
    """
    Enhanced Quiescence search with:
    - Check extensions (Phase 1.3)
    - Dynamic depth based on position volatility (Phase 1.2)
    - Delta pruning
    ...
    # Hard limit on qsearch depth to prevent explosion
    if q_ply > MAX_QSEARCH_PLY and not in_check:
        return evaluate(board)
    
    # If in check, we must escape - can't stand pat
    if in_check:
        best = -MATE_SCORE + ply
        # Generate ALL moves when in check, not just captures
        ...
        for move in moves:
            ...
            score = -quiescence(board, -beta, -alpha, info, ply + 1, q_ply + 1)
            unmake_move(board)
    ...
    # Generate moves - include checks in early qsearch
    ...
    for move in moves:
        ...
        # In early qsearch (q_ply <= MAX_QSEARCH_CHECK_PLY), also search checks
        if not is_cap and not is_promo:
            # Check if this move gives check (only in early qsearch)
            if q_ply <= MAX_QSEARCH_CHECK_PLY:
                if not make_move(board, move):
                    continue
...
def alpha_beta(board: BoardState, depth: int, alpha: int, beta: int,
    ...
    """
    V5c Alpha-Beta with ultra-aggressive pruning for maximum depth:
    - Ultra-aggressive LMR with pre-computed tables
    - Ultra-aggressive Late Move Pruning (LMP)
    ...
    # Check Extension (minimal for V5c speed)
    # Only extend in very shallow searches to avoid horizon effects
    if in_check and depth <= 3 and ply < MAX_PLY - 16:
        depth += 1
    ...
    # =========================================================================
    # REVERSE FUTILITY PRUNING (ultra-aggressive in V5c)
    # =========================================================================
    ...
    # =========================================================================
    # NULL MOVE PRUNING (ultra-aggressive in V5c)
    # =========================================================================
    if (do_null and depth >= NULL_MOVE_MIN_DEPTH and not in_check and not pv_node):
        ...
        if has_pieces:
            # V5c: Ultra-aggressive null move reduction for maximum speed
            R = NULL_MOVE_R + (depth > 5) + (depth > 10) + (depth > 15)  # Progressive reduction

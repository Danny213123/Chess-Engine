...
function App() {
  ...
  const switchEngine = async (ver, color) => {
    ...
    try {
      ...
      const details = e?.response?.data?.detail || e?.message || String(e);
      setError(details);
      setEngineStatus("");
      log(`Error switching engine: ${details}`);
    } finally {
      ...
      <div className="info-panel">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div className="engine-config">
            {engineStatus && <div className="status-message">{engineStatus}</div>}
            <select
              value={whiteEngine}
              onChange={(e) => switchEngine(e.target.value, "white")}
            ...
            <select
              value={blackEngine}
              onChange={(e) => switchEngine(e.target.value, "black")}
              disabled={engineSwitching}
            >
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
            ...
            />
            <button onClick={loadFen} style={{ flexShrink: 0, minWidth: '72px' }}>Load</button>
          </div>
